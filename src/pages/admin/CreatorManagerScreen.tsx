    ...
    <View className="bg-white rounded-lg border border-gray-200 p-4 mb-3">
      <View className="flex-row items-center justify-between mb-2">
        <View className="flex-row items-center flex-1">
          <View className={`w-6 h-6 rounded-full items-center justify-center mr-2 ${getTierColor(item.tier)}`}>
            <Text className="text-xs font-bold">{item.tier}</Text>
          </View>
      ...
      <Text className="text-sm text-gray-600 mb-1">{item.tiktokHandle}</Text>
      <Text className="text-sm text-gray-600 mb-2">{item.category} • {item.niche.join(', ')}</Text>
      ...
      <View className="flex-row items-center justify-between">
        ...
        <View className="flex-row items-center">
          ...
          <Text className="text-sm text-gray-600">{item.engagement}% engagement</Text>
        </View>
      </View>
    </View>
  );
      ...
      <ScrollView className="flex-1 bg-gray-50 p-4">
        <View className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
          ...
          <View className="flex-row space-x-4 mb-4">
            <View className="flex-1">
              <FormField
                ...
                onChangeText={(text) => setFormData({...formData, followers: text})}
                placeholder="850K"
              />
            </View>
            <View className="flex-1">
              <FormField
                ...
                onChangeText={(text) => setFormData({...formData, engagement: parseFloat(text) || 0})}
                placeholder="4.2"
          ...
          <FormField
            ...
            onChangeText={(text) => setFormData({...formData, gmv: parseInt(text) || 0})}
            placeholder="125000"
            keyboardType="numeric"
          />
          ...
          <View className="flex-row items-center justify-between mb-6">
            <Text className="text-sm font-medium text-gray-700">Verified Creator</Text>
            <Pressable
            ...
            >
              <Text className={`font-medium ${formData.isVerified ? 'text-blue-800' : 'text-gray-600'}`}>
          ...
          <Text className="text-lg font-bold text-gray-900 mb-4">Example Videos</Text>
          
          {(formData.exampleVideos || []).map((video, index) => (
            <View key={index} className="bg-gray-50 rounded-lg p-4 mb-4">
              <View className="flex-row items-center justify-between mb-3">
                <Text className="font-medium text-gray-900">Video {index + 1}</Text>
              ...
              <FormField
                label="Video Title"
                value={video.title}
                onChangeText={(text) => updateExampleVideo(index, 'title', text)}
              ...
              <FormField
                ...
                onChangeText={(text) => updateExampleVideo(index, 'thumbnail', text)}
                placeholder="https://..."
                keyboardType="url"
              />
              ...
              <FormField
                ...
                value={video.url}
                onChangeText={(text) => updateExampleVideo(index, 'url', text)}
              ...
              <View className="flex-row space-x-4">
                <View className="flex-1">
                  <FormField
                    ...
                    onChangeText={(text) => updateExampleVideo(index, 'views', parseInt(text) || 0)}
                    placeholder="1000000"
                    keyboardType="numeric"
                  />
                </View>
                <View className="flex-1">
                  <FormField
                    ...
                    onChangeText={(text) => updateExampleVideo(index, 'likes', parseInt(text) || 0)}
                    placeholder="50000"
                    keyboardType="numeric"
                  />
                </View>
          ...
          >
            ...
            <Text className="text-gray-500 font-medium mt-1">Add Example Video</Text>
          </Pressable>
    ...
    <View className="flex-1 bg-gray-50">
      <View className="flex-row items-center justify-between p-4">
        ...
        >
          ...
          <Text className="text-white font-medium ml-1">Add Creator</Text>
        </Pressable>
      </View>
